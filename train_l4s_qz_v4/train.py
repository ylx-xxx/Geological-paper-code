...
from dataset import Landslide4SenseDataset, read_h5_first
...
def build_positive_patch_sampler(dataset):
    """
    Oversample patches containing landslide pixels.
    This improves exposure to minority landslide regions without changing the loss aggressively.
    """
    weights = []
    pos_patch = 0
    empty_patch = 0

    print("[Info] Building positive patch weighted sampler...")

    for _, mask_path in tqdm(dataset.samples, desc="sample weights", ncols=120):
        mask = read_h5_first(mask_path)
        mask = np.asarray(mask)
        if mask.ndim == 3:
            mask = np.squeeze(mask)

        pos_ratio = float(np.mean(mask > 0))

        # Softer sampling strategy:
        # Avoid over-amplifying landslide patches.
        if pos_ratio <= 0.0:
            w = 0.80
            empty_patch += 1
        elif pos_ratio < 0.005:
            w = 1.15
            pos_patch += 1
        elif pos_ratio < 0.02:
            w = 1.35
    ...
    print(f"[Info] positive patches: {pos_patch}")
    print(f"[Info] empty patches:    {empty_patch}")
    print(f"[Info] weight min/max:   {min(weights):.3f} / {max(weights):.3f}")
...
def main():
    ...
    args.weighted_sampler = bool(args.weighted_sampler)
    ...
    train_sampler = build_positive_patch_sampler(train_set) if args.weighted_sampler else None
